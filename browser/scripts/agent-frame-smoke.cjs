...
const child = `<!doctype html><style>body{font:16px sans-serif;margin:18px}button,input{font:16px sans-serif;margin:8px;padding:8px}</style><div style="height:900px"></div><label>Frame name <input aria-label="Frame name" oninput="document.getElementById('input-status').value=String(event.isTrusted)"></label><button aria-label="Frame action" onclick="document.querySelector('#status').textContent='Clicked';const b=document.createElement('button');b.setAttribute('aria-label','Frame dynamic');b.textContent='Frame dynamic';document.body.append(b)">Frame action</button><span id="status">Idle</span><input id="input-status" aria-label="Input status" value="Idle">`;
